...
- (NSString *)replacingPattern:(NSString *)pattern template:(NSString *)myTemplate {
    if (![self isNotBlank]) {
        return self;
    }
    ...
    return [regularExpression stringByReplacingMatchesInString:self options:0 range:NSMakeRange(0, self.length) withTemplate:myTemplate];
}

- (NSArray <NSTextCheckingResult *>*)matchesPattern:(NSString *)pattern {
    if (![self isNotBlank]) {
        return @[];
    }
    NSRegularExpression *regularExpression = [NSRegularExpression regularExpressionWithPattern:pattern options:NSRegularExpressionCaseInsensitive error:nil];
    return [regularExpression matchesInString:self options:NSRegularExpressionCaseInsensitive range:NSMakeRange(0, self.length)];
    
}

- (Boolean)isExistPattern:(NSString *)pattern {
    return [self matchesPattern:pattern].count;
}
